...
function extractXhsUrl(text: string): string | null {
  ...
  for (const pattern of patterns) {
    ...
    if (match) {
      return match[1].replace(/[,，。！]/g, "").replace(/^xhslink\.com/, "http://xhslink.com");
    }
  }
  return null;
}
...
function shouldPerformOcr(body: string): boolean {
  ...
  return !(body.length > MAX_LENGTH || itineraryPatterns.some((p) => p.test(body)));
}
...
export async function POST(req: Request) {
  try {
    ...
    const imageRes = await fetch("https://tools.mgtv100.com/external/v1/pear/xhsImg", {
      method: "POST",
    ...
    if (shouldOcr) {
      for (const url of images) {
        const ocrRes = await fetch("https://tools.mgtv100.com/external/v1/pear/ocr", {
          method: "POST",
          headers: { "Content-Type": "application/x-www-form-urlencoded" },
    ...
    return NextResponse.json({ title, body, images, ocrTexts }, { status: 200 });
  } catch (err: any) {
    return NextResponse.json({ error: err.message || "解析错误" }, { status: 500 });
  }
}
